...
FROM page
INNER JOIN categorylinks AS c ON page_id=c.cl_from AND c.cl_to!="%s"
INNER JOIN categorylinks AS cc on page_id=cc.cl_from AND cc.cl_to="%s"
WHERE c.cl_to NOT REGEXP "%s"
GROUP BY c.cl_to
ORDER BY COUNT(page_id) DESC
LIMIT 50;
""" % (BASE_CAT, BASE_CAT, '|'.join(BAD_CATS)))
...
page = pywikibot.Page(site, "%s/categories" % TOP_PAGE)
page.put(table)
...
WHERE page_namespace=6 AND cl_to = '%s' AND rev_user_text NOT REGEXP '[Bb]ot'
GROUP BY 1
HAVING edit_count>0 
ORDER BY 2 DESC;""" % BASE_CAT
)
...
for userx, count in cursor.fetchall():
    ...
    table.append([count, "<abbr title='%s edits to %s'>[[User talk:%s|%s]]</abbr>" % (count, PROJECT, userx, userx)])
...
pywikibot.setAction("Update")
page = pywikibot.Page(site, "%s/volunteers" % TOP_PAGE)
...
pywikibot.setAction("Update")
page = pywikibot.Page(site, "%s/most_edited" % TOP_PAGE)
...
url = "https://tools.wmflabs.org/glamtools/glamorous.php?doit=1&category=%s&use_globalusage=1&show_details=1&projects[wikipedia]=1&projects[wikimedia]=1&projects[wikisource]=1&projects[wikibooks]=1&projects[wikiquote]=1&projects[wiktionary]=1&projects[wikinews]=1&projects[wikivoyage]=1&projects[wikispecies]=1&projects[mediawiki]=1&projects[wikidata]=1&projects[wikiversity]=1&format=xml" % BASE_CAT
...
if len(images)>10:  # If less than 10 I think something went wrong.
    pywikibot.setAction("Update")
    page = pywikibot.Page(site, "%s/usage" % TOP_PAGE)
    page.put(report)
